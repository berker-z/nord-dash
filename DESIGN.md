...
- Login auth feedback: Google Identity Services load failures and popup failures surface in the existing inline auth error banner. Keep the sign-in button disabled while GIS is unavailable or an auth popup is pending, with concise uppercase status text.
