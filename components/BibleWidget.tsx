    ...
    <div className="h-full flex flex-col font-mono">
      ...
      <form onSubmit={handleSubmit} className="mt-4 relative group">
        <textarea
          ...
          className="w-full bg-nord-1 border-2 border-nord-3 rounded-lg pl-4 pr-12 py-3 text-base focus:outline-none focus:border-nord-8 focus:bg-nord-2 placeholder-nord-3 min-h-[80px] resize-y text-nord-4 transition-colors"
          disabled={loading}
        />
        <button 
          type="submit" 
        ...
        >
          {loading ? <div className="w-5 h-5 border-2 border-nord-4 border-t-transparent rounded-full animate-spin" /> : <Send size={20} />}
        </button>
      </form>
    </div>
  );
